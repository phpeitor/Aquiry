...
const au = "State Machine 6",
	...
	vm = (rt = {}) => {
		const {
			...
			...rt
		}), [B, Mt] = hl.useState(""), [Ht, Nt] = hl.useState(""), [Y, vt] = hl.useState(Hd), [_showReq, _setShowReq] = hl.useState(!1), [isChecking, setIsChecking] = hl.useState(!1), it = hl.useRef(null), ne = hl.useRef(!1), ce = hl.useRef(!1);
		const successTriggers = [
			"isAmaze",
			"amazeamazeamaze transition",
			"amazeamazeamaze",
			"isProud",
			"proud transition",
			"proud",
			...
			"happy",
			"happy (big)",
			"happy to idle transition",
			"happy to idle(big)"
		];
		const errorTriggers = [
			"isHardstop",
			"hard stop transition",
			"hard stop",
			"isDisagree",
			"disagree transition",
			"disagree",
			"isEmbarassed",
			"embarassed transition",
			"embarassed",
			"isEmpathetic",
			"empathy transition",
			"empathy"
		...
		const passwordTriggers = [
			...
			"big think",
			"think",
			"think and thinking loop",
			"thinking loop",
			"thinking to idle"
		];
		const getStateMachineInput = name => {
			if (!I || typeof I.stateMachineInputs != "function") return null;
			const inputs = I.stateMachineInputs(au) || [];
			return inputs.find(input => input && input.name === name) || null;
		};
		const fireNamedTrigger = name => {
			if (!name) return;
			const input = getStateMachineInput(name);
			if (input && input.type === "Trigger" && typeof input.fire == "function") {
				requestAnimationFrame(() => input.fire());
				return !0;
			}
			if (I && typeof I.play == "function" && Array.isArray(I.animationNames) && I.animationNames.includes(name)) {
				requestAnimationFrame(() => I.play(name));
				return !0;
			}
			return !1;
		};
		const triggerRandom = names => {
			const list = (names || []).filter(Boolean);
			if (!list.length) return;
			const choice = list[Math.floor(Math.random() * list.length)];
			const fired = fireNamedTrigger(choice);
			if (!fired) {
				for (const name of list) {
					if (fireNamedTrigger(name)) break;
				}
			}
		};
		...
		const triggerPassword = () => {
			for (const name of passwordTriggers) {
				if (fireNamedTrigger(name)) return;
			}
			const input = getStateMachineInput("isThinking");
			if (!input) return;
			if (input.type === "Boolean") {
				input.value = !0;
				setTimeout(() => {
		...
		const triggerSuccess = () => triggerRandom(successTriggers);
		const triggerError = () => triggerRandom(errorTriggers);
		hl.useEffect(() => {
			it?.current && !it.current.dataset.ready && it.current.focus && it.current.focus()
		}, [it]);
		const xt = Dt => {
				const It = Dt.target.value;
				Mt(It);
				if (It.trim().length > 0) triggerPassword();
			},
			Yt = () => {
				triggerPassword();
			};
					...
					children: Rn.jsxs("form", {
						...
						children: [Rn.jsx("label", {
							...
							children: Rn.jsx("input", {
								...
								onChange: Dt => {
									Nt(Dt.target.value);
									if (Dt.target.value.trim().length > 0) triggerPassword();
								}
							})
						}), Rn.jsx("button", {
							className: "login-btn",
