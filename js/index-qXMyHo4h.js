...
const au = "State Machine 1",
	...
	vm = (rt = {}) => {
		const {
			...
			src: "./resources/logininteraction.riv",
			stateMachines: au,
			autoplay: !0,
		...
		hl.useEffect(() => {
			if (isFocus && isFocus.type === "Boolean" && !B.trim().length) setBooleanInput(isFocus, !1)
		}, [B]);
		hl.useEffect(() => {
			if (isPassword && isPassword.type === "Boolean" && !Ht.trim().length && !document.activeElement?.matches('input[name="password"]')) setBooleanInput(isPassword, !1)
		}, [Ht]);
		hl.useEffect(() => {
			if (eyeTrack && eyeTrack.type === "Number") setNumberInput(eyeTrack, 0)
		}, [B, Ht]);
		hl.useEffect(() => {
			if (ne.current) {
				fireTrigger(loginFail), ne.current = !1
			}
		}, [loginFail]);
		hl.useEffect(() => {
			if (ce.current) {
				fireTrigger(loginSuccess), ce.current = !1
			}
		}, [loginSuccess]);
		const xt = Dt => {
				const It = Dt.target.value;
				Mt(It);
				setBooleanInput(isFocus, !!It.trim().length);
				setNumberInput(eyeTrack, It.trim().length)
			},
			Yt = () => {
				setBooleanInput(isFocus, !0)
			},
			xm = (Dt, It = !1) => {
					...
					children: Rn.jsxs("form", {
						...
						children: [Rn.jsx("label", {
							children: Rn.jsx("input", {
								...
								onBlur: () => {
									setBooleanInput(isFocus, !1);
									setBooleanInput(isPassword, !!Ht.trim().length);
								},
								ref: it
							})
						}), Rn.jsx("label", {
